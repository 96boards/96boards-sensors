...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:key name="comp" match="comp" use="concat(footprint, '|', fields/field[@name='Seeed SKU'], '|', fields/field[@name='Mfr Part #'])"/>
    ...
    <xsl:template match="components">
        <xsl:for-each select="comp[generate-id(.) = generate-id(key('comp', concat(footprint, '|', fields/field[@name='Seeed SKU'], '|', fields/field[@name='Mfr Part #']))[1])]">
	    <xsl:sort select="@ref"/>
            <xsl:variable name="SKU" select="concat(footprint, '|', fields/field[@name='Seeed SKU'], '|', fields/field[@name='Mfr Part #'])" />
            ...
            <xsl:text>,</xsl:text>
            <xsl:value-of select="count(key('comp', concat(footprint, '|', fields/field[@name='Seeed SKU'], '|', fields/field[@name='Mfr Part #'])))"/><xsl:text>,</xsl:text>
